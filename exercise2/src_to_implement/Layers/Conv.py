...
class Conv(BaseLayer):
    ...
    def pad_2d_forward(self, input_array, padded_value=0):

        return self.execute_padding(input_array, padded_value, self.pad_y, self.as_y, self.pad_x, self.as_y)
    ...
    def dilate_err_tensor(self, err_tensor, axis1=0, axis2=1):
        if self.is1dConv:
            if self.stride_shape[0] == 1:
                return err_tensor
            else:
                val_1 = err_tensor.shape[axis1] - 1
                for i in range(self.stride_shape[0] - 1):
                    err_tensor = np.insert(err_tensor,
                                           tuple([(i + 1) * (x + 1) for x in range(val_1)]), 0,
                                           axis=axis1)

        else:
            if self.stride_shape[0] == 1 and self.stride_shape[1] == 1:
                return err_tensor
            else:
                val_1 = err_tensor.shape[axis1] - 1
                ...
                val_2 = err_tensor.shape[axis2] - 1
                for i in range(self.stride_shape[1] - 1):
                    err_tensor = np.insert(err_tensor,
                                           tuple([(i + 1) * (x + 1) for x in range(val_2)]), 0,
                                           axis=axis2)
        return err_tensor
    ...
    def stride_index(self):

        if self.is1dConv:
            [row] = self.input_tensor.shape[2:]

            if self.stride_shape[0] == 1:
                value = np.ones((1, row + 1))
            else:
                value = (np.arange(1, row + 1)) % self.stride_shape[0]

        else:
            [row, col] = self.input_tensor.shape[2:]

            if self.stride_shape[0] == 1:
                value_x = np.ones((1, row + 1))
            else:
                value_x = (np.arange(1, row + 1)) % self.stride_shape[0]

            if self.stride_shape[1] == 1:
                value_y = np.ones((1, col + 1))
            else:
                value_y = (np.arange(1, col + 1)) % self.stride_shape[1]

            value = value_y * value_x[:, None]
