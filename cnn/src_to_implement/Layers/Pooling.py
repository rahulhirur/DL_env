...
class Pooling(BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        self.pooling_output_height = int(1 + (self.input_tensor.shape[2] - self.pooling_shape[0]) / self.stride_shape[0])
            ...
            for c in range(self.input_tensor.shape[1]):
                for hi in range(self.pooling_output_height):
                    for wi in range(self.pooling_output_width):
                        ...
                        backward_output[b, c, 
                                        hi * self.stride_shape[0]: hi * self.stride_shape[0] + self.pooling_shape[0], 
                                        wi * self.stride_shape[1]: wi * self.stride_shape[1] + self.pooling_shape[1]][self.hi_m, self.wi_m] 
                        += error_tensor[b, c, hi, wi]
